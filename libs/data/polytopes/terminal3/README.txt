...
To install place the "terminal3" directory inside
...
The "terminal3" directory contains the database of all three-dimensional
canonical Fano polytopes. This classification is derived in:

    [Kas]  Alexander M. Kasprzyk, "Toric Fano threefolds with terminal
            singularities", Tohoku Mathematical Journal, 58 (2006),
            no. 1, 101-121.

Each entry in the database has been assigned a unique ID in the range 1..634.
These ID correspond with the IDs on the Graded Ring Database:

    [GRDB] http://grdb.lboro.ac.uk/forms/toricf3t

The format of the files in the "terminal3" directory is deliberately
