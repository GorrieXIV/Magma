...
ProcessCharacters := procedure(~CP, C, mask, sqr_lev:LLLSort := false)
    ...
    */
	...
	LLLzeit := Cputime();
	L, T, r := LLLGram(m:Delta:=CP`LLLDelta,Sort:=false,InitialSort:=LLLSort);
...
ProcessIrreds := procedure(~CP, i, I)
    ...
    L, T, r := LLLGram(m:Delta:=CP`LLLDelta,Sort:=false,InitialSort:=false);
    blk`Reds cat:= I;
    CP`CSBlocks[i] := blk;
...
CyclicInduction := procedure(~CP, i)
    ...
    ProcessCharacters(~CP, b, IndexedSetToSet(up), 0);
...
AddPermChtr := procedure(~CP:Power := 4)
    ...
    L, T, r := LLLGram(M:Delta:=0.999, Sort := false);
    int_chtrs := [];
    for i := 1 to r do
...
CheckIrreds := procedure(~CP, level)
    ...
    G := CP`G;
    sos := 0;
    N := LMGOrder(G) div LMGOrder(LMGCentre(G));
...
vprintf Chtr: "CS = %o, %o irreducible(s) known (out of %o)\n", [b`MaxRank: b in
 CP`CSBlocks], #KnownIrreducibles(G), #cl;
    ...
    // "Calling LMGSylow on centralizer, ", p;
    // C:Minimal;
	    S := LMGSylow(C,p);
...
vprintf Chtr: "CS = %o, %o irreducible(s) known (out of %o)\n", [b`MaxRank: b in
 CP`CSBlocks], #KnownIrreducibles(G), #cl;
	...
	lat := LatticeWithGram(blk`Gram);
	sv := [t[1]:t in ShortVectors(lat, 1, 1:Max := r)];
	for v in sv do
...
function ChtrTableWrapper(G:ClassSizeLimit:=1, Al :="Default", Check:=2);
    if #ClassesData(G) eq #KnownIrreducibles(G) then 
	T := CharacterTableDS(G);
	CheckCharacterTable(T, 0);
	return T;
    end if;
    if Al eq "Default" then 
	if #G le 5000 then
	    T := CharacterTableDS(G);
	    CheckCharacterTable(T, Check);
	    return T;
	...
	elif Type(G) ne GrpPC and LMGIsSoluble(G) then
	    P,f := LMGPCGroup(G);
	    ...
	    return ChtrTable(G:ClassSizeLimit:=ClassSizeLimit, Check:=Check);
	end if;
    elif Al eq "DS" then
	T := CharacterTableDS(G);
	CheckCharacterTable(T, Check);
...
intrinsic LMGCharacterTable(G :: GrpMat:Al := "Default", DSSizeLimit:=1, Check := 2) -> SeqEnum
{Character Table of finite group G}
    return ChtrTableWrapper(G:Al:=Al,ClassSizeLimit:=0, Check:=Check);
end intrinsic;
