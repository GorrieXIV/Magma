...
EVAL := procedure(~G,~CC,~SS,~MUSS,~stuff, ~P)
    ...
    zeit := Cputime();
    ProcessElementaryCharacters(~stuff`CP, ~stuff`Chtrs, ~stuff`fusion, ~stuff`sizes, ~stuff`mults, ~stuff`mask, ~stuff`stop, chtrs);
...
ProcessElementaryCharacters := procedure(~CP, ~chtrs, ~fusion, ~sizes, ~mults, ~mask, ~stop, subgp_chtrs)
    clG := CP`Classes;
    V := CP`V;
