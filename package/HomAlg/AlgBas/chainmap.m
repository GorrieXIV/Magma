...
if n gt 0 then
   ...
   for i := 1 to nn do
    if &+prj1`BettiNumbers[a1-d-i+1] eq 0
        and &+prj2`BettiNumbers[a2-i+1] ne 0
      then xx := <ZeroMatrix(ff,0,
                &+[prj2`BettiNumbers[a2-i+1][j]*


                        Dimension(ProjectiveModule(alg,j))
                :j in [1 .. NumberOfProjectives(alg)]]),
                prj1`BettiNumbers[a1-d-i+1],
                prj2`BettiNumbers[a2-i+1]>;
    elif &+prj1`BettiNumbers[a1-d-i+1] ne 0
        and &+prj2`BettiNumbers[a2-i+1] eq 0
      then xx := <ZeroMatrix(ff,
                &+[prj1`BettiNumbers[a1-d-i+1][j]*
                        Dimension(ProjectiveModule(alg,j))
                :j in [1 .. NumberOfProjectives(alg)]],0),
                prj1`BettiNumbers[a1-d-i+1],
                prj2`BettiNumbers[a2-i+1]>;
    elif &+prj1`BettiNumbers[a1-d-i+1] eq 0
        and &+prj2`BettiNumbers[a2-i+1] eq 0
      then xx := <ZeroMatrix(ff,0,0),
                prj1`BettiNumbers[a1-d-i+1],
                prj2`BettiNumbers[a2-i+1]>;
    else
      map1 := <prj1`ResolutionRecord[a1-d-i+1],prj1`BettiNumbers[a1-d-i+1],
                   prj1`BettiNumbers[a1-d-i+2]>;
      map2 := <prj2`ResolutionRecord[a2-i+1],prj2`BettiNumbers[a2-i+1],
                   prj2`BettiNumbers[a2-i+2]>;
...
for j := 1 to #b1 do
   if &+b1[j] eq 0 then 
      Plst[j] := ZeroModule(BasicAlgebra(BaseRing(alg)));
   else 
      Plst[j] := DirectSum([SS:i in [1 .. &+b1[j]]]);
   end if;
end for;
Qlst := [];
for j := 1 to #b2 do
   if &+b2[j] eq 0 then
      Qlst[j] := ZeroModule(BasicAlgebra(BaseRing(alg)));
   else
      Qlst[j] := DirectSum([SS:i in [1 .. &+b2[j]]]);
   end if;
end for;
...
if &+ddd eq 0 then
   rrr := recformat<ChainMapRecord:List, ChainDegrees:SeqEnum,
          TopsOfCohomologyGenerators:List, TopsOfCohomologyChainMaps:List,
          ProjectiveResolution: Rec>;
   bbb := rec<rrr|ChainMapRecord := [* *], ChainDegrees := [],
          TopsOfCohomologyGenerators := [* *], 
	  TopsOfCohomologyChainMaps := [* *],
          ProjectiveResolution := Prj>;
...
ddd := [TP1[i][3]: i in [1 .. #TP1]];
rrr := recformat<ChainMapRecord:List, ChainDegrees:SeqEnum,
	  TopsOfCohomologyGenerators:List, TopsOfCohomologyChainMaps:List,
	  ProjectiveResolution: Rec>; 
bbb := rec<rrr|ChainMapRecord := AC, ChainDegrees := ddd,
          TopsOfCohomologyGenerators := TP1, TopsOfCohomologyChainMaps := TP2,
          ProjectiveResolution := Prj>;
...
intrinsic DegreesOfGenerators(ACM::Rec) -> SeqEnum
{Given the generators for cohomolgy, the function returns the list 
of degrees of the minimal generators.}

return ACM`ChainDegrees;

end intrinsic;

//*********************************************************************

intrinsic CohomologyRightModuleGenerators(P::Rec , Q::Rec, CQ::Rec) -> Tup
...
AC := [* *];
TP1 := [* *];                               //tops of generators
TP := CQ`TopsOfCohomologyGenerators cat CQ`TopsOfCohomologyChainMaps; 
                          //tops of cohomology for T
                                   // the tops will be triples consisting
                                   // the actual list of matrices of the tops,
                                   // the ordered list of product of generators
                                   // and the total degree.
...
uuu1 :=  Maximum(
	[i:i in [1 .. #P`BettiNumbers]|&+Reverse(P`BettiNumbers)[i] ne 0])-1;
uuu1 :=  Maximum(
        [i:i in [1 .. #Q`BettiNumbers]|&+Reverse(Q`BettiNumbers)[i] ne 0])-1;
ff := BaseRing(alg);
...
if &+ddd eq 0 then
   rrr := recformat<ChainMapRecord:List, ChainDegrees:SeqEnum,
          TopsOfCohomologyGenerators:List>;
   bbb := rec<rrr|ChainMapRecord := [* *], ChainDegrees := [],
          TopsOfCohomologyGenerators := [* *]>;
       return bbb;
end if;
...
for i := 1 to #ddd do
   if ddd[i] ne 0 then
      if i lt nn1 + nn2 or nn2 eq 0 then
         lss := ListSummandIndices(xxx[i]);
         aaa := [j: j in [1 .. #lss]|lss[j] eq nn];
         for t in aaa do
            chm := nChainMap(P,Q,t,i,#P`BettiNumbers-1,alg);
            Append(~AC,chm);
            ch, tp := TopOfChainMap(chm,alg,xx,Q`BettiNumbers,i);
            Append(~TP1,<tp,[#TP1+1],i>);
         end for;                                         // t
      else
                          // get all of the tops
...
deg := [AC[i][#AC[i]][2]:i in [1 .. #AC]];
rrr := recformat<ChainMapRecord:List, ChainDegrees:SeqEnum,
	  TopsOfCohomologyGenerators:List>; 
bbb := rec<rrr|ChainMapRecord := AC, ChainDegrees := deg,
          TopsOfCohomologyGenerators := TP1>;
...
intrinsic CohomologyLeftModuleGenerators(P::Rec ,CP::Rec, Q::Rec) -> Tup
{Given projective resolutions P and Q for simple module S and T over a basic
algebra A and the cohomology generators for T associated to the resolution
Q, the function returns the chain maps in compact form of the minimal 
...
AC := [* *];
TP1 := [* *];                               //tops of generators
TP := CP`TopsOfCohomologyGenerators cat CP`TopsOfCohomologyChainMaps;
                            //tops of cohomology for S
                                   // the tops will be triples consisting
                                   // the actual list of matrices of the tops,
                                   // the ordered list of product of generators
                                   // and the total degree.
         ...
         if &+[ddd[s] + #[x:x in dde|x eq i-s]:s in [1 .. i-1]] ge 2 then
            for c := 1 to #TP1 do
               for d := 1 to #TP do
                  if TP1[c][3]+TP[d][3] eq i then
                     ww := Vector(Transpose(
                           ModuleMap(TP[d][1],TP1[c][3]+TP[d][3])*
			   ModuleMap(TP1[c][1],TP1[c][3]))*rho);
                     if ww notin W then
                        W := sub<V|W,ww>;
                        top :=  TP[d][1]*TP1[c][1];
                     end if;                            // ww notin W
...
intrinsic DegreesOfCohomologyGenerators(AC::Rec) -> SeqEnum
...
intrinsic CohomologyGeneratorToChainMap(P::ModCpx,Q::ModCpx,C::Rec,
		  n::RngIntElt) -> MapChn
...
intrinsic CohomologyGeneratorToChainMap(P::ModCpx,C::Rec,n::RngIntElt) ->
     MapChn
