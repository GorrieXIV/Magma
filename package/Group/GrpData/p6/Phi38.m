...
/* Easterfield list for Phi38 */

EasterfieldPhi38 := function (p)
   ...
   for x in GF (p) do
   Q := quo < F | family, [
                  Alpha[5]^p = Alpha[1]^-(Z!x),
                  Alpha[6]^p = Alpha[1]^-(Z!((x-1) / 3))
              ] >;
   Append (~Pres, Q);
   end for;
