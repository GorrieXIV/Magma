...
ExtendOneSubgroupModN := function(G,N,pQ,NtoQ,A,B,subrec,maxind,calcpres:
                                            Print:=0,Maximal:=false)
 ...
 * If Maximal is true, then only whole group and maximal subgps returned
 * In this "mod N" version, the relations of the record have to be
 * multiplied by listed elements of N
 * Unfortunately we need pQ and NtoQ at one point.
 */
 local RF, Z, ExtendSubsH, ES, ESR, M, K, dim, p, mindim, phi, RM, L,
       SL, GonSLgens, gen, perm, im, Rep, repgen, S, SA, NA, ngs,
       ...
       quoSMtoG, subSMtoG, modifiers,
       liftgens, liftpres, liftsub, vec,
 ...
 Z := Integers();
 ExtendSubsH := [];
       /* This will be the lifted subgroup-list to be returned. */
 ...
 dim := Dimension(M);
 S:=subrec`subgroup;
 SA := sub< Generic(G) |S,N,A>;
 NA := sub< Generic(G) | N,A >;
 ngs:=Ngens(S);
 AssertAttribute(SA, "Order", #G div subrec`index);
 mindim := Minimum(
        [ d : d in [0..dim] | subrec`index * p^(dim-d) le maxind ] );

 if mindim eq dim or (Maximal and subrec`index ne 1) then
   src := subrec;
   
   liftgens := SA eq NA select [M.i @@ phi : i in [1..dim]]
   ...
   if SA eq NA then
     ...
     src`modifiers := [ Id(G) : i in [1..#Relations(src`presentation)] ];
   else 
     ...
     for i in [1..#rels] do
       rel := rels[i];
       w := ((LHS(rel)*RHS(rel)^-1)@quoSsubmap) * subrec`modifiers[i];
       Append(~RVS,subSpresM!Eltseq(M!(w@phi)));
     end for;
/*
     for rel in rels do
          w := LHS(rel)*RHS(rel)^-1;
...
*/
     ...
     nrels := #Relations(src`presentation);
     src`modifiers cat:= [ Id(G) : i in [1..(nrels - #rels)] ];
   end if;
   return [src];
 end if;
 ...
 if Maximal then //only want maximal submodules
   SL := [ s : s in SL | s eq RM or IsIrreducible(quo<RM|s>) ];
 end if;
 ...
 /* Now we find all liftings to G/NB of the specified subgroup of G/NA */
 ...
 for i in [1.. #rels] do
   rel := rels[i];
   w := LHS(rel)*RHS(rel)^-1;
   w := (w@Spresmap) * subrec`modifiers[i]; 
   Append(~RV,w @ phi);
 end for;
 nlifts:=0;
 for rep in orbreps do
   ...
   /* We are now going to find those subgroups of G/NB which intersect
    * NA/NB in the subgroup phi^-1(intmorph).
   ...
   if not split then
	...
	assert #Comps eq nc;
       ...
       if Print gt 2 then
         print "        ",
                     #normgens,"normalising generators for this subgroup.";
       end if;
...
/* Unfortunately need to use quotient here */
       ...
       for i in [1..#normgens] do
         ...
         for j in [1..ngs] do
           ...
           word[i][j]:=w@wgtoqSp;
           gelt := S.j^-1*gen*(w@wgtoG)*gen^-1;
           //need to get rid of unwanted element of N
           gelt := gelt * (gelt @ pQ @@ NtoQ)^-1;
           vec :=
             (SM!(gelt@phi))@projSM;
     ...
     for compno in [1..#Compreps] do
       ...
       liftgens := [];
       if NA ne G or subdim eq 0 then 
         for i in [1..ngs] do
           gen := S.i;
       ...
       liftgens := SA eq NA select subSMtoG else liftgens cat subSMtoG;
       ...
       if (calcpres) then
         if subdim ne 0 then
           if SA eq NA then
             ...
             nrels := #Relations(liftpres);
             modifiers := [ Id(G) : i in [1..nrels] ];
           else
             ...
             for i in [1..#rels] do
               rel := rels[i];
               w := LHS(rel)*RHS(rel)^-1;
               w := (w@quoSsubmap) * subrec`modifiers[i]; 
               Append(~RVS,subSpresM!Eltseq(subSM!SM!(w@phi)));
             end for;
             liftpres := ModuleExtension(subSpresM,RVS);
             nrels := #Relations(liftpres);
             modifiers :=
               subrec`modifiers cat [ Id(G) : i in [1..(nrels - #rels)] ];
           end if;
         else
           liftpres := Spres;
           modifiers := subrec`modifiers;
         end if;
         Append(~ES,rec<RF | subgroup:=liftsub, presentation:=liftpres,
			order:=subrec`order * p^subdim, length:=classlen,
                  modifiers:=modifiers, index := subrec`index * p^quodim >);
       else
     ...
     if  nc gt 1 and normsubS ne SA and subrec`order ge quolimit then
       ...
       ngp := [];
       sublist := {@ sub<Generic(G)|e`subgroup,N,B> :  e in ES @};
