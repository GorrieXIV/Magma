...
PGL2ActionOnTransvectionGroups := function (L, phi, tau, x : PSLAction := true)
     ...
     null_spaces := [Nullspace (MA!w - Identity (MA)) : w in H_Images];
     if exists{n : n in null_spaces | Dimension (n) lt 1} then 
        vprint su3: "Wrong dimension of null spaces"; 
        return false, _, _;
     end if;
     points := [n.1: n in null_spaces];
     ...
     if (PSLAction) then
         ...
         ox:=Order(x);
         assert ox eq q+1;
         killme := (BaseRing(H)!((h^ox)[1,1]))^-1;
         j:= Log(killme);
         assert mu^j eq killme;
         d:= Gcd(ox, q-1);
         assert j mod d eq 0;
         gcd:=Gcd(ox, j);
         R:=ResidueClassRing((q-1) div d);
         ox:= R!(ox div d);
         j:= R!(j div d);
         i:= (ox^-1)*j;
         i:= Integers()!i;
         j:=Integers()!j*d;
         ox:= Order(x);     
         assert ((i*ox - j) mod (q-1)) eq 0;
         list := [i + ((q-1) div d)*t : t in [0..d-1]];
         assert forall{ t : t in list | ((t*ox - j) mod (q-1)) eq 0};
         list:= [mu^t : t in list];
         assert exists(alpha) {alpha : alpha in list | Order((Generic(H)![alpha, 0, 0, alpha])*h) eq ox};
         mymatrix:= Generic(H)![alpha, 0, 0, alpha];
         return true, mymatrix * h, _;
     end if;
end function;
