...
OmegaMinusChar2WordInGen := function (G, A)
   ...
   LF := ext < F | 2>;
   ww := PrimitiveElement (LF);
   /*
   if ww^(q+1) ne w then
      ww := find_quad_ext_prim(F, LF);
      poly := MinimalPolynomial(ww, F);
      LF := ext<F | poly>;
      assert PrimitiveElement(LF)^(q + 1) eq w;
   end if;
   */  
/* 
try 
   ...
   "OmegaMinusChar2WordInGen: Problem with choice of primitive element for field";
end try;
*/
   sl := SL(2, #F^2);
   e := Degree (F);
   ...
   GetAOne := procedure (~A, ~S1, ~S2)
      ...
      /* if A[1, 1] or A[1, 2] are the only non-zero entries in first column */
   ...
   /* Once we kill the 3rd to (d-2)-th positions on the top row of A, 
      this function will kill of the d-1 and d slots on the top row. 
   ...
   /* Once we kill the 3rd to (d-2)-th positions in the first column of A, 
      this function will kill of the d-1 and d slots in the first column. 
      As A preserves an orthogonal form, the 2nd slot is also killed. */
   ...
   if d eq 2 then
      ...
      /* Let BB be the image of B in SL(2, q^2). 
         x tensor y is BB tensor BB^q. 
         As scalars can be carried across the 2 factors, 
         x may be a multiple of BB. 
         Hence, we divide x by Root (det, 2) to get BB.*/
      ...
      if d ne 4 then
         ...
         _, wword := SLWordInGen (sl, sl!x);
         phi := hom<Parent (wword) -> SS | S^(SS.5^-1), S^(SS.5^-1), SS.1, SS.3>;
      end if;
