...
{return the Leedham-Green and O'Brien standard generators for the quasisimple 
 ...
 string type is one of SL, Sp, SU, Omega, Omega+, Omega-
   ...
   if type eq "Omega" then require IsOdd (d) and IsOdd (q): "Dimension and field size must be odd"; end if;
   if type in {"Omega+", "Omega-"}  then require (IsEven (d) and d ge 4): 
      "Dimension must be even and at least 4"; end if;
    
   case type:
       when "SL": return SLChosenElements (SL(d, q)); 
       when "Sp": return SpChosenElements (Sp(d, q)); 
       when "SU": return SUChosenElements (SU(d, q)); 
       when "Omega": return SOChosenElements (Omega(d, q): SpecialGroup := SpecialGroup); 
       when "Omega+": return PlusChosenElements (OmegaPlus(d, q): SpecialGroup := SpecialGroup); 
       when "Omega-": return MinusChosenElements (OmegaMinus(d, q): SpecialGroup := SpecialGroup); 
   end case;
end intrinsic;
...
{return the Leedham-Green and O'Brien presentation on standard generators 
 ...
 and the relations as SLPs in this group are returned
   ...
   case type:
       when "SL": return int_StandardPresentationForSL (d, q: Projective := Projective);
       when "Sp": return int_StandardPresentationForSp (d, q: Projective := Projective);
       when "SU": return int_StandardPresentationForSU (d, q: Projective := Projective);
/* 
       when "Omega+": return int_StandardPresentationForOmega (d, q: Projective := Projective, Type := "Omega+");
       when "Omega-": return int_StandardPresentationForOmega (d, q: Projective := Projective, Type := "Omega-");
       when "Omega": return int_StandardPresentationForOmega (d, q: Type := "Omega");
*/
   end case;
end intrinsic;
