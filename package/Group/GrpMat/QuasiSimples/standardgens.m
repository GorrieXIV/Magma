...
//A and B where B has order 3,  and AB has order 53.
   ...
   when "2^2O8p3":
     ...
     until sct lt 50 or ct eq nt;
   when "O8p4":
     //not in ATLAS - this is very bad - must try and improve
     z,a,A := F(34);
     if not z then return false,_,_; end if;
     a := a^17; A := A^17;
     z,b,B := F(7);
   ...
   when "O10p3":
     //not in ATLAS - very bad indeed!
     z,a,A := F(82);
     if not z then return false,_,_; end if;
     //41st power is in Class 2A
     a := a^41; A := A^41;
     z,b,B := F(11);
     if not z then return false,_,_; end if;
     bb := b; BB := B;
     ct:=0; repeat ct+:=1;  c,C := R(); b := bb^c; B := BB^C;
     until (O(a*b) eq 11 and O(a*b^2) eq 12 and O(a*b*a*b^2) eq 30 and
          O(a*b*a*b^2*a*b^3) eq 30) or ct eq 500*nt;
     if ct eq 500*nt then return false,_,_; end if;
   when "O10m3":
     //not in ATLAS - very bad indeed!
     z,a,A := F(26);
     if not z then return false,_,_; end if;
     ...
     bb := b; BB := B;
     ct:=0; repeat ct+:=1;  c,C := R(); b := bb^c; B := BB^C;
     until (O(a*b) eq 41 and O(a*b^2) eq 24 and O(a*b*a*b^2) eq 41 and
          O(a*b*a*b^2*a*b^3) eq 52) or ct eq 500*nt;
     if ct eq 500*nt then return false,_,_; end if;
   when "O12p2":
     //not in ATLAS
     z,a,A := F(34);
