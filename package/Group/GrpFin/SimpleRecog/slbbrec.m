...
// Inform("");
    ...
    for t in x do if t[1] le d and t[2] le d then
	M[t[1]][t[2]] := t[3];
    end if; end for;
    return M;
end function;
...
// Inform("SL2ReOrder");
    ...
    newT := [one, t];
    newT := [i le 2 select newT[i]
		    else auto(bbg, Self(i-1), pow, conj): i in [1 .. p^e]];
    Tgamma := [x^j: x in newT];
    ...
    if i gt 2 then
	Tgamma := [one] cat Tgamma[i .. p^e] cat Tgamma[2 .. i - 1];
    end if;
...
//Derek Holt added extra parameter xismat to avoid error in case when
//bbg element happens to be a matrix - 1/9/04.
...
// Inform("SLConstructBasisQ");
    ...
    T := [one, b1];
    T := [i le 2 select T[i] else sinv * Self(i-1) * s: i in [1 .. q]];
    Tgamma := [one, b1prime];
    Tgamma := [i le 2 select Tgamma[i] else s * Self(i-1) * sinv: i in [1..q]];
    baseQ := [i eq 1 select b1 else Self(i - 1) ^ c: i in [1 .. d-1]];
    baseQgamma := [i eq 1 select b1prime else Self(i-1) ^ c: i in [1 .. d-1]];
