...
intrinsic CondensedAlgebra(A::AlgMat : Rand := 0, limprod := 7, limsum := 8) -> AlgMat
...
"GOT Rand:", Rand;
A;
[Rank(A.i): i in [1 .. Ngens(A)]];
*/

    if Rand cmpeq 0 then
	if forall{i: i in [1 .. Ngens(A)] | IsUnit(A. i)} then
	    Rand := "Meataxe";
//"CHOOSE Meataxe";
	else
	    Rand := "PartSpin";
//"CHOOSE PartSpin";
	   ...
	   "Error in parameter Rand: Unknown algorithm specified.";
    end if;
...
lim1 := limprod;
lim2 := limsum;
R := AlgebraGenerators(A : Rand := rand, limprod := lim1, limsum := lim2);
