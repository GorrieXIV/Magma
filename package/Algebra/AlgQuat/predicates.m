...
import "../AlgAss/order-jv.m": RadicalIdealizer;
...
function EichlerFast(O, p)
  ...
  case Type(R):
    ...
    // Would need to check if elements are squares in the padic completion.
    if IsEven(Norm(p)) then return 0; end if;
    // Maybe IsSquare is too expensive?
    f:= func< x | y eq 0 select 0 else (IsSquare(y) select 1 else -1) where y:= h(x) >;
  end case; 

  B:= Generators(O);
  for i in [1..20] do
    x:= &+[ B[j] * (Random(k) @@ h) : j in [1..#B] ];
...
function EichlerInternal(O, p)
  // Let's try some discriminants
  e:= EichlerFast(O, p);
  if e ne 0 then return e; end if;

  // So the result is probably 0 or p is an even prime. Let's check to be sure.
  A:= Algebra(O);
  if Type(O) eq AlgQuatOrd then
    ...
    B:= [ u^Valuation(b[1], p) * b[2] : b in P ];
  end if;
...
intrinsic TernaryForm(O::AlgAssVOrd) -> ModDed, AlgMatElt
{"} //"
  ok, str:= IsSupported(O);
  require ok : str;
  lvl, Dual:= Gorenstein_Internal(O);
  ...
  T:= Matrix(3, [ Trace(x*Conjugate(y))/2 : x,y in NN ]);
  V:= VectorSpace(FF, 3);
...
intrinsic QuaternionOrder(M::ModDed, T::AlgMatElt) -> AlgAssVOrd
{Returns the quaternion order associated to the module M and the ternary quadratic form T.}
  require Ncols(T) eq 3 and IsSymmetric(T) and Rank(T) eq 3: "The form must be a symmetric, ternary and non-degenerate";
  R:= BaseRing(M); 
  K:= NumberField(R);
  ok, T:= CanChangeRing(T, K);
  require ok : "The form must be over the same ring as the module";
  ...
  X:= [ p[2] * g: g in Generators(p[1]), p in PseudoBasis(M) ];
  S:= ChangeUniverse(X, V);
  S:= [ h(s): s in S ];

  N:= M * T * Transpose(M) where M:= Matrix(K, Matrix(X));
  n:= { N[i,j] * (i eq j select 1 else 2) : j in [1..i], i in [1..Nrows(N)] };
  n:= ideal< R | n >^-1;
  T:= [C ! 1] cat [ g*s*t : g in Generators(n), s,t in S ];
