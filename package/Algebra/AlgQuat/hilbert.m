...
forward EvenHilbertSymbol, EvenDiagonalNormForm;
...
intrinsic HilbertSymbol(a::FldAlgElt, b::FldAlgElt, p::RngOrdIdl) -> RngIntElt
  ...
  // When p is odd, this is an extremely simple bit of arithmetic. 
  ...
  // p is even
  return HilbertSymbol(QuaternionAlgebra<K | a,b>, p);
end intrinsic;
...
intrinsic HilbertSymbol(A::AlgQuat, p::RngOrdIdl) -> RngIntElt
  ...
  if IsOdd(Minimum(p)) then
    ...
    // Call more complicated (local) routine
    // (only return the boolean)
    return s where s is EvenHilbertSymbol(A,p);
  end if;
end intrinsic;
