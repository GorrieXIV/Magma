...
/*
intrinsic ArithmeticRadical(O::AlgAssVOrd, p::.) -> AlgAssVOrdIdl
...
//intrinsic RadicalIdealizer(O::AlgAssVOrd, p::. : Side:= "Intersection") -> AlgAssVOrd
//{The radical idealizer of O at the prime (ideal) p}
function RadicalIdealizer(O, p: Side:= "Intersection");
  ok, I:= _Radical(O, p);
//  require ok: I;
  ...
  if T in {FldRat, FldFunRat} then
    ...
    if assigned A`FactoredDiscriminant then
      ...
      if assigned A`MaximalOrder then
        ...
        D:= [ f[1] : f in Factorization( ideal< Integers(F) | 2*a*b> ) | HilbertSymbol(A, f[1]) eq -1 ];
      end if;
      assert IsEven(#S+#D);
