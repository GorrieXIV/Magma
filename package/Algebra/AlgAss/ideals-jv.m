...
intrinsic 'in'(a::RngElt, I::AlgAssVOrdIdl) -> BoolElt
  ...
  if Type(I) eq AlgQuatOrdIdl then
    ...
    ans:= forall{ i: i in [1..4] | coords[i] in ideals[i] };
  end if;
...
function has_element_of_norm(ZB, Nnu : all:=false, svp:=true, prune:=false)
  ...
  // The trick here ensures we are looking for short vectors 
  // (a shortest vector in the special case where nu will generate the ideal)

  alpha := F! 1/Nnu;
  // make integral
  bool, dalpha := IsIntegral(alpha);
  if not bool then
    alpha *:= dalpha;
  end if;

  // Twisted trace form
  //     Tr( alpha * trd(x*y^bar) )
  // Here the mat mults are a bit costly when field degree is 2
  // (would probably improve if everything was over Z)

  M    := Matrix(4*Degree(F), &cat [Flat(x) : x in ZB]);
  R    := RepresentationMatrix(alpha); 
