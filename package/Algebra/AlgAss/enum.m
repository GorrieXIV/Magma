...
/* // this should work, but is too slow in practice.
      for u in U do
        ok, Ns:= NormEquation(MaximalOrder(NF), u: Exact:= true);
        if not ok then continue; end if;
        for n in Ns do
          xx:= e[2] * x + e[1] where e:= Eltseq(NF ! n);
          if xx in O then
            Append(~NewUnits, xx);
            Exclude(~U, u);
...
get_coords:= function(O, p)
  if Type(O) eq AlgQuatOrd then
    ...
    B:= [ O | ];
    for b in PseudoBasis(O) do
      F:= Factorization(b[1]);
      if IsEmpty(F) then
        Append(~B, b[2]);
      else
        if forall{ f : f in F | f[1] ne p } then Append(~F, < p, 0 >); end if;
        x:= WeakApproximation( [f[1] : f in F], [ f[2] : f in F] );
        Append(~B, b[2] * x );
      end if;
    end for;
...
RightIdealClassesAndOrders:= function(O : Support:= [], strict_support:= true, 
  ...
  if flat then
    ...
    U, Umap := IndependentUnits(Z_F : Al:="ClassGroup");
...
intrinsic ConjugacyClasses( O :: AlgAssVOrd : Al:= "AllOrders") -> SeqEnum
  ...
  if IsDefinite(A) then
    ...
    /* Don't want to maintain this any more
    if Al eq "FixOrder" then
      Ideals:= Type(BaseRing(O)) eq RngInt select RightIdealClasses(O) else RightIdealClasses(O : Al:= "FixOrder");
      Orders:= [O];
      for I in Ideals do
        LO:= LeftOrder(I);
        if forall{OO : OO in Orders | not IsIsomorphic(LO, OO)} then
          Append(~Orders, LO);
        end if;
      end for;
    end if;
    */
    ...
    for p in Primes do
      ...
      Append(~Orders, LeftOrder(rideal<O | [Inverse(phi)(M2F.1)] cat Generators(p)>));
    end for; 
  end if;
  return Orders;
end intrinsic;
