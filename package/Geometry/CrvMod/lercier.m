...
declare verbose ModularIsogeny, 3;

forward ArtinSchreierRoot, MakeRelations;

function Lercier(E,F,ell)
    // Lercier's algorithm for computing a factor of the ell'th 
    // division polynomial of the curve E, where E and F are 
    // ell-isogenous.

    vprint ModularIsogeny, 1: "Using Lercier's algorithm.";
    RelationsTyme := Cputime();
    ...
    alpha := Root(a, 4);
    // Have to invert alpha later.
    if alpha eq 0 then return false, P!0; end if;
    beta := Root(b, 4);
    if ell eq 3 then 
    ...
    while true do       
        ...
        if poly_trace eq 1 then 
            ...
            vprint ModularIsogeny, 2: "New relation =", poly_trace;
    ...
    vprint ModularIsogeny, 2: "Found", #rels - Rank(Q), 
        "trivial relations among", Rank(Q), "unknowns."; 
    rels cat:= MakeRelations(p,d,alpha,R);
    vprint ModularIsogeny, 1:
       "Relation construction time:", Cputime(RelationsTyme);

    vprint ModularIsogeny, 1: "Solving for points.";
	...
	vprint ModularIsogeny, 2: "Trivial solution:", point;
	IndentPop();
    ...
    vprint ModularIsogeny, 3: "Number of monomials:", #mons;
	...
	vprintf ModularIsogeny, 3:
	    "Relation %o; rank: %o, columns: %o\n", k, r, #c;
	Ar := Nrows(A);
    ...
    vprint ModularIsogeny, 2:
	"Matrix construction time:", Cputime(VARIETY_TIME);
    ...
    repeat
	vprint ModularIsogeny, 3: "Nullity:", Nrows(B);
	if Nrows(B) eq 0 or B[1, 1] ne 1 then
	    vprint ModularIsogeny, 2: "No solution";
	    IndentPop();
	    return false, P!0; 
	end if;
	...
	vprint ModularIsogeny, 3: "Number of new relations:", #new;
