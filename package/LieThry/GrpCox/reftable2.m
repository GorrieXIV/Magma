...
for i:=2 to rank do
 for j:=1 to i-1 do
  if mx[i][j] le 0 then
   ...
   Include(~eroots, {@ [j,0,1,4], [i,1] @});
   act[j][i]:=#eroots;
   act[j][#eroots]:=i;
   act[i][#eroots]:=#eroots;
   Include(~eroots, {@ [j,1], [i,0,1,4] @});
   act[i][j]:=#eroots;
   act[i][#eroots]:=j;
   act[j][#eroots]:=#eroots;
  elif mx[i][j] eq 5 then
   Include(~eroots, {@ [j,0,1,5], [i,1] @});
   act[j][i]:=#eroots;
   act[j][#eroots]:=i;
   Include(~eroots, {@ [j,1], [i,0,1,5] @});
   act[i][j]:=#eroots;
   act[i][#eroots]:=j;
   Include(~eroots, {@ [j,0,1,5], [i,0,1,5] @});
   act[j][#eroots-1]:=#eroots;
   act[j][#eroots]:=#eroots-1;
   act[i][#eroots-2]:=#eroots;
   act[i][#eroots]:=#eroots-2;
  else
   Include(~eroots, {@ [j,0,1,mx[i][j]], [i,1] @});
   act[j][i]:=#eroots;
   act[j][#eroots]:=i;
   Include(~eroots, {@ [j,1], [i,0,1,mx[i][j]] @});
   act[i][j]:=#eroots;
   act[i][#eroots]:=j;
   Include(~eroots, {@ [j,2,0,mx[i][j]], [i,0,1,mx[i][j]] @});
   act[j][#eroots-1]:=#eroots;
   act[j][#eroots]:=#eroots-1;
   Include(~eroots, {@ [j,0,1,mx[i][j]], [i,2,0,mx[i][j]] @});
   act[i][#eroots-3]:=#eroots;
   act[i][#eroots]:=#eroots-3;
   ii:=3;
   while 2*ii lt mx[i][j] - 1 do
    Include(~eroots, {@ [j,ii,0,mx[i][j]], [i,ii-1,0,mx[i][j]] @});
    act[j][#eroots-1]:=#eroots;
    act[j][#eroots]:=#eroots-1;
    Include(~eroots, {@ [j,ii-1,0,mx[i][j]], [i,ii,0,mx[i][j]] @});
    act[i][#eroots-3]:=#eroots;
    act[i][#eroots]:=#eroots-3;
    ii +:= 1;
   end while;
   if IsOdd(mx[i][j]) then
    Include(~eroots, {@ [j,ii,0,mx[i][j]], [i,ii,0,mx[i][j]] @});
    act[j][#eroots-1]:=#eroots;
    act[j][#eroots]:=#eroots-1;
    act[i][#eroots-2]:=#eroots;
    act[i][#eroots]:=#eroots-2;
...
// If there are 2 or more bonds, then there is a circuit created.
    ...
    if numjoins eq 0 then
     ...
     nextrt:=Include(eroots[i],[k,0,1,mx[k][x]]);
     ind:=Index(eroots,nextrt);
     if ind eq 0 then
      Include(~eroots,nextrt);
      act[k][i]:=#eroots;
...
// Why is cfx[3] defined? Or do the if statements apply even if the entry is undefinend?
  ...
  if #eroots ge counter+500 then
   counter:=#eroots;
   vprint VerboseGrpCox, 6 : i, counter;
  end if;
 end for;
 i +:= 1;
end while;
